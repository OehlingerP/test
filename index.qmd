...
```{r}
current_training_status <- get_training_status(df_notes, Sys.Date())
...
ggplot(df_status, aes(x = date, y = value)) + 
  ...
  theme_minimal() +
  scale_color_manual(values = c("GREEN" = "#3a4f01", 
                                "YELLOW" = "#F49937",
                                "GRAY" = "#93B7BE", 
                                "RED" = "#800000")) +
  theme(axis.text.y = element_blank(), 
...
plot <- x_type %>%
  ...
  theme_minimal() +
  scale_fill_manual(values = gxplot::gx_colors()) +
