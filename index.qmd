...
ls_rules_daily <- jsonlite::fromJSON("data/daily-rules.json", simplifyDataFrame = F)$daily
ls_notes <- prepare_workout_data(gs4_find("Workout Notes")$id)
ls_notes <- lapply(ls_notes, function(x) x %>% filter(date < Sys.Date()))
```
...
dates <- dates[dates > min(dates) + 1]

ls <- sapply(dates, function(DATE) {
...
ggplot(df_status, aes(x = date, y = as.numeric(priority))) + 
  geom_line() +
  geom_point(mapping = aes(color = action, shape = action), size = 5) +
  facet_wrap(.~status_type) +
  theme(axis.text.y = element_blank()) +
  theme_minimal() +
  scale_color_manual(
    values = setNames(df_colors$color, 
                      df_colors$action)) +
  theme(axis.text.y = element_blank(), 
